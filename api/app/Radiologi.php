    ...
    if (is_writeable($folder_structure)) {
      ...
      for ($a = 0; $a < count($_FILES['fileList']); $a++) {
        ...
        if (!empty($_FILES['fileList']['tmp_name'][$a])) {
          ...
          $ext = pathinfo($_FILES['fileList']['name'][$a], PATHINFO_EXTENSION);

          if (move_uploaded_file($_FILES['fileList']['tmp_name'][$a], '../document/radiologi/' . $parameter['uid_radiologi_order'] . '/' . $nama_lampiran . '.' . $ext)) {
            ...
            array_push($result['response_upload'], 'Gagal diupload : ' . $_FILES['fileList']['tmp_name'][$a] . ' => ' . $nama_lampiran . '-' . $a . '.' . $ext);
          }
        }
      }
    } else {
