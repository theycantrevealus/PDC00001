...
<script type="text/javascript">
    $(function() {
        ...
        $("body").on("click", ".NRSItems", function() {
            var currentIDNRS = $(this).attr("id").split("-");
            currentIDNRS = currentIDNRS[currentIDNRS.length - 1];
            var currentMenu = $("a[data-toggle=\"tab\"].active").attr("href");
            var mySlider;
            if(currentMenu === "#tab-assesment-awal-igd-1") {
                mySlider = sliderIGDBiasa;
            } else {
                mySlider = sliderIGDBidan;
            }
                ...
                success: function(response) {
                    if (response.response_package != ""){
                        ...
                        $.each(MetaData.pasien, function(key, item){
                            ...
                            if(key === "nama") {
                                $("#target_pasien").html(item).parent().attr({
                                    "href": __HOSTNAME__ + "/igd/perawat/index/" + uidPasien + "/" + MetaData.antrian.kunjungan + "/" + MetaData.antrian.departemen
                                });
