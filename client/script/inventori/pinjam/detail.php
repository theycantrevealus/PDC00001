<script type="text/javascript">
    $(function() {
        $.ajax({
            async: false,
            url: __HOSTAPI__ + "/Inventori/lend_detail/" + __PAGES__[3],
            beforeSend: function(request) {
                request.setRequestHeader("Authorization", "Bearer " + <?php echo json_encode($_SESSION["token"]); ?>);
            },
            type: "GET",
            success: function(resp) {
                var resp = resp.response_package.response_data[0];
                
                var itemDetail = resp.detail;
                load_supplier("#txt_supplier", {
                    uid: resp.penerima.uid,
                    nama: resp.penerima.nama
                });

                if(resp.diajukan.uid !== __ME__) {
                    $("#btnSubmitReturn").attr({
                        "disabled": "disabled"
                    }).addClass("btn-danger").removeClass("btn-success");
                }

                $("#txt_tujuan").val(resp.keterangan);
                for(var a in itemDetail) {
                    autoItem({
                        item: {
                            uid: itemDetail[a].item.uid,
                            nama: itemDetail[a].item.nama
                        },
                        batch: {
                            uid: itemDetail[a].batch.uid,
                            name: itemDetail[a].batch.batch
                        },
                        qty: itemDetail[a].qty,
                        satuan: itemDetail[a].item.satuan_terkecil_info.nama
                    });
                }
            },
            error: function(resp) {
        ...
        function autoItem(setter = {
            ...
            $(newQty).inputmask({
                ...
                autoGroup: false,
                digitsOptional: true
            }).addClass("form-control qty_check").val(setter.qty);

            load_product(newSelectpr, setter.item);
            $(newSelectpr).addClass("form-control item");

            $(newBatch).append("<option title=\"" + setter.batch.name.toUpperCase() + "\" value=\"" + setter.batch.uid + "\">" + setter.batch.name.toUpperCase() + "</option>");
            $(newBatch).select2("data", {id: setter.batch.uid, text: setter.batch.name.toUpperCase()});
            $(newBatch).trigger("change");

            $(newCelllSatuan).html(setter.satuan);
        ...
        function load_product(target, selectedData = {
            uid: '',
            nama: ''
        }, appendData = true) {
			var selected = [];
            ...
            if(selectedData.uid !== '') {
                $(target).append("<option title=\"" + selectedData.nama + "\" value=\"" + selectedData.uid + "\">" + selectedData.nama + "</option>");
                $(target).select2("data", {id: selectedData.uid, text: selectedData.nama});
        ...
        function load_supplier(target, selectedData = {
            uid: '',
            nama: ''
        }) {
			var kategoriData;
